  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      try {
        ...
        console.warn("[accounting-sync] Warning: Could not sync customer:", err.message);
      }
...
export const getClientFinancialSummary = async (req, res) => {
  ...
  try {
    ...
    // Aggregate Accounting Invoices & Payments
    let totalInvoiced = 0;
    let totalPaid = 0;
    let outstandingBalance = 0;

    const customer = client.accountingCustomer || (await prisma.customers.findFirst({ where: { clientId: id } }));

    if (customer) {
      const invoices = await prisma.customer_invoices.findMany({
        where: { customer_id: customer.id, state: { not: "cancelled" } },
        select: { amount_total: true, paid_amount: true, amount_due: true },
      });
      totalInvoiced = invoices.reduce((sum, inv) => sum + Number(inv.amount_total || 0), 0);
      totalPaid = invoices.reduce((sum, inv) => sum + Number(inv.paid_amount || 0), 0);
      outstandingBalance = invoices.reduce((sum, inv) => sum + Number(inv.amount_due || 0), 0);
    }
