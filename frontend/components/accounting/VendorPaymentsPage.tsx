...
export default function VendorPaymentsPage() {
  ...
  async function save(event?: FormEvent, postAfterSave = false) {
    ...
    try {
      ...
      accountingToast(postAfterSave ? 'Vendor payment posted successfully' : `Draft vendor payment ${selected ? 'updated' : 'created'} successfully`); setOpen(false); await load();
    } catch (error) { accountingToast(message(error), 'error'); } finally { setSaving(false); }
  }
  async function post(payment: VendorPayment) {
    setSaving(true); try { await vendorPaymentApi.post(payment.id); accountingToast('Vendor payment posted successfully'); setPendingAction(null); await load(); } catch (error) { accountingToast(message(error), 'error'); } finally { setSaving(false); }
  }
  async function remove(payment: VendorPayment) {
    setSaving(true); try { await vendorPaymentApi.remove(payment.id); accountingToast('Draft vendor payment deleted'); setPendingAction(null); await load(); } catch (error) { accountingToast(message(error), 'error'); } finally { setSaving(false); }
  }
  ...
  return (
    <AccountingPageShell section="Payables" title="Vendor Payments" description="Record payments and allocate them to posted vendor bills.">
    ...
    <Dialog open={open} onOpenChange={(value) => !saving && setOpen(value)}><DialogContent className="max-h-[94vh] max-w-[calc(100vw-2rem)] overflow-y-auto xl:max-w-[1400px]"><DialogHeader><DialogTitle>{readonly ? 'Vendor payment' : selected ? 'Edit vendor payment' : 'Vendor payment'}</DialogTitle><DialogDescription>Enter the payment details and select the outstanding bills to pay.</DialogDescription></DialogHeader><form onSubmit={save} className="space-y-5">
      <fieldset disabled={readonly || saving} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="Reference" value={form.reference} set={(value) => setForm({ ...form, reference: value })} optional={!Boolean(selectedMethod?.requires_reference)} />
      </fieldset>
      ...
      <div className="ml-auto w-full rounded-2xl border p-4 sm:max-w-md"><h3 className="mb-3 font-semibold">Payment Summary</h3><div className="space-y-2 text-sm"><div className="flex justify-between"><span>Payment Amount</span><b>{money(form.amount)}</b></div><div className="flex justify-between"><span>Selected Bill Total</span><b>{money(selectedBillTotal)}</b></div><div className="flex justify-between"><span>Allocated</span><b>{money(allocated)}</b></div><div className={`flex justify-between ${Math.abs(unallocated) > 0.005 ? 'text-secondary' : ''}`}><span>Unallocated</span><b>{money(unallocated)}</b></div><div className="flex justify-between border-t pt-2"><span>Balance After Payment</span><b>{money(balanceAfterPayment)}</b></div><div className="flex justify-between"><span>Status</span><b className={paymentStatus === 'Paid' ? 'text-emerald-700' : paymentStatus === 'Partially Paid' ? 'text-secondary' : 'text-muted-foreground'}>{paymentStatus}</b></div></div></div>
