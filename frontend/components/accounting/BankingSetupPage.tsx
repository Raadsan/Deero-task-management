...
import { SquarePen, Landmark, Plus, RefreshCw, Trash2 } from 'lucide-react';
...
export default function BankingSetupPage() {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      accountingToast(`Bank account ${selected ? 'updated' : 'created'} successfully`); setOpen(false); await load();
    } catch (error) { accountingToast(errorMessage(error), 'error'); } finally { setSaving(false); }
  }
  async function remove(row: Row) {
    setSaving(true); try { await bankAccountApi.remove(row.id); accountingToast('Bank account deleted'); setDeleteTarget(null); await load(); } catch (error) { accountingToast(errorMessage(error), 'error'); } finally { setSaving(false); }
  }
  ...
  return (
    <AccountingPageShell
      section="Banking"
      title="Bank Accounts"
      description="Manage payment institutions, account details, and ledger links in one place."
    >
    ...
    <Dialog open={open} onOpenChange={(value) => !saving && setOpen(value)}>
      <DialogContent className={accountingDialogWideClass}>
        <DialogHeader className={configDialogHeaderClass}>
          <DialogTitle>{selected ? 'Edit' : 'Add'} bank account</DialogTitle>
          <p className="text-sm text-zinc-500">Configure the institution and accounting connection for this account.</p>
        </DialogHeader>
        <form onSubmit={save}>
          <div className={`${configDialogBodyClass} grid gap-4 sm:grid-cols-2`}>
          ...
          <DialogFooter className={configDialogFooterClass}>
            <Button type="button" variant="outline" onClick={() => setOpen(false)} className={btnFormCancel}>Cancel</Button>
            <Button type="submit" disabled={saving} className={btnFormSubmit}>{saving ? 'Saving...' : 'Save'}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
    <AccountingConfirmDialog open={Boolean(deleteTarget)} title="Delete Bank Account" description="Confirm removal of this bank account record." confirmLabel="Delete Account" destructive busy={saving} details={deleteTarget && <div className="flex justify-between"><span className="text-zinc-500">Name</span><b>{String(deleteTarget.account_name)}</b></div>} onCancel={() => setDeleteTarget(null)} onConfirm={() => deleteTarget && void remove(deleteTarget)} />
    </AccountingPageShell>
  );
}
...
function Actions({ edit, remove }: { edit: () => void; remove: () => void }) { return <div className="flex justify-end gap-1"><button onClick={edit} className={actionBtnEdit}><SquarePen className="size-4" /></button><button onClick={remove} className={actionBtnDelete}><Trash2 className="size-4" /></button></div>; }
function Field({ label, value, set, optional }: { label: string; value: string; set: (value: string) => void; optional?: boolean }) { return <label className="text-sm font-medium text-zinc-700">{label}{!optional && ' *'}<input required={!optional} value={value} onChange={(event) => set(event.target.value)} className={`mt-1 ${accountingFormFieldClass}`} /></label>; }
function Select({ label, value, set, rows, labelKey = 'name', account, optional }: { label: string; value: string; set: (value: string) => void; rows: Row[]; labelKey?: string; account?: boolean; optional?: boolean }) { return <label className="text-sm font-medium text-zinc-700">{label}{!optional && ' *'}<select required={!optional} value={value} onChange={(event) => set(event.target.value)} className={`mt-1 ${accountingFormSelectClass}`}><option value="">Select {label.toLowerCase()}</option>{rows.map((row) => <option key={row.id} value={row.id}>{account ? `${String(row.code || '')} - ${String(row.name || '')}` : String(row[labelKey] || row.name || '')}</option>)}</select></label>; }
