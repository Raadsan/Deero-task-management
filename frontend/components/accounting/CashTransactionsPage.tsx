...
import { ArrowDownToLine, ArrowUpFromLine, SquarePen, Eye, Plus, RefreshCw, Send, Trash2 } from 'lucide-react';
...
export default function CashTransactionsPage() {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      accountingToast(`Cash ${form.type} saved as draft`); setOpen(false); await load();
    } catch (error) { accountingToast(message(error), 'error'); } finally { setSaving(false); }
  }
  async function post(entry: Row) { setSaving(true); try { await journalEntryApi.post(entry.id); accountingToast('Cash transaction posted'); setPendingAction(null); await load(); } catch (error) { accountingToast(message(error), 'error'); } finally { setSaving(false); } }
  async function remove(entry: Row) { setSaving(true); try { await journalEntryApi.remove(entry.id); accountingToast('Draft cash transaction deleted'); setPendingAction(null); await load(); } catch (error) { accountingToast(message(error), 'error'); } finally { setSaving(false); } }
  const columns: DashboardTableColumn<Row>[] = [
    ...
    { key: 'status', header: 'Status', align: 'center', cell: (row) => <span className={`${dashboardStatusBadgeClass} ${row.state === 'posted' ? 'bg-emerald-600 text-white' : 'bg-secondary/100 text-white'}`}>{String(row.state)}</span> },
    { key: 'actions', header: 'Actions', align: 'right', cell: (row) => <div className="flex justify-end gap-1"><button onClick={() => edit(row, true)} className={actionBtnView}><Eye className="size-4" /></button>{row.state === 'draft' && <><button onClick={() => edit(row)} className={actionBtnEdit}><SquarePen className="size-4" /></button><button onClick={() => setPendingAction({ type: 'post', entry: row })} className={actionBtnView}><Send className="size-4" /></button><button onClick={() => setPendingAction({ type: 'delete', entry: row })} className={actionBtnDelete}><Trash2 className="size-4" /></button></>}</div> },
  ];

  return (
    <AccountingPageShell section="Banking" title="Cash Transactions" description="Record cash and bank receipts or payments with balanced ledger entries.">
    ...
    <AccountingConfirmDialog open={Boolean(pendingAction)} title={`${pendingAction?.type === 'delete' ? 'Delete' : 'Post'} Cash Transaction`} description={pendingAction?.type === 'delete' ? 'Confirm removal of this draft cash transaction.' : 'Confirm this cash transaction before posting it to the ledger.'} confirmLabel={`${pendingAction?.type === 'delete' ? 'Delete' : 'Post'} Transaction`} destructive={pendingAction?.type === 'delete'} busy={saving} details={pendingAction && <div className="flex justify-between"><span className="text-zinc-500">Reference</span><b>{String(pendingAction.entry.reference || pendingAction.entry.entry_number || '—')}</b></div>} onCancel={() => setPendingAction(null)} onConfirm={() => pendingAction && void (pendingAction.type === 'delete' ? remove(pendingAction.entry) : post(pendingAction.entry))} />
    </AccountingPageShell>
  );
}
