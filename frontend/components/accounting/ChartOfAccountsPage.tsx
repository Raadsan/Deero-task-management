...
import { BookOpen, Check, ChevronLeft, ChevronRight, ChevronsUpDown, SquarePen, Eye, Plus, RefreshCw, Search, Trash2, WalletCards } from 'lucide-react';
...
export default function ChartOfAccountsPage() {
  ...
  return (
    <AccountingPageShell section="General Ledger" title="Chart of Accounts" description="Build and maintain the account structure used throughout your books.">
      ...
      <DashboardDataTable rows={filtered} columns={columns} loading={loading} searchValue={query} onSearchChange={setQuery} searchPlaceholder="Search code, account name, or type..." emptyText="No accounts found" minWidth="1000px" action={<button type="button" onClick={createAccount} className={btnCreatePage}><Plus className="size-4" /> Add Account</button>} filters={<><select value={codeFilter} onChange={(event) => setCodeFilter(event.target.value)} className={dashboardSelectClass}><option value="all">All codes</option>{['1', '2', '3', '4', '5'].map((digit) => <option key={digit} value={digit}>Code {digit}</option>)}</select><FilterSelect label="All account types" value={typeFilter} onChange={setTypeFilter} options={accountTypes.map((row) => ({ value: String(row.id), label: String(row.name) }))} /><FilterSelect label="All companies" value={companyFilter} onChange={setCompanyFilter} options={companies.map((row) => ({ value: String(row.id), label: String(row.name) }))} /><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className={dashboardSelectClass}><option value="all">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select><button onClick={() => void loadData()} disabled={loading} className="flex size-[42px] items-center justify-center rounded-md border"><RefreshCw className={`size-4 ${loading ? 'animate-spin' : ''}`} /></button></>} />
      ...
      <Dialog open={viewOpen} onOpenChange={setViewOpen}><DialogContent className="sm:max-w-3xl"><DialogHeader><DialogTitle>{String(selected?.code || '')} — {String(selected?.name || 'Account details')}</DialogTitle><DialogDescription>Review this chart of account record.</DialogDescription></DialogHeader><div className="grid gap-3 sm:grid-cols-2"><DetailCard label="Account Type" value={String(typeMap.get(Number(selected?.account_type_id))?.name || '—')} /><DetailCard label="Company" value={String(companyMap.get(Number(selected?.company_id))?.name || '—')} /><DetailCard label="Status" value={selected?.is_active === false ? 'Inactive' : 'Active'} /><DetailCard label="Manual Entry" value={selected?.allow_manual_entry === false ? 'Not allowed' : 'Allowed'} /><DetailCard label="Reconcilable" value={selected?.is_reconcilable ? 'Yes' : 'No'} /><DetailCard label="Notes" value={String(selected?.notes || '—')} /></div><DialogFooter><button type="button" onClick={() => setViewOpen(false)} className="h-10 rounded-md border border-zinc-200 px-5 text-sm font-semibold">Close</button>{selected && <button type="button" onClick={() => { setViewOpen(false); editAccount(selected); }} className="h-10 rounded-md bg-primary px-5 text-sm font-semibold text-white">Edit Account</button>}</DialogFooter></DialogContent></Dialog>
