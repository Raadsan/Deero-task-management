...
export default function CreditNotesPage() {
  ...
  async function post(note: CreditNote) {
    setSaving(true); try { await creditNoteApi.post(note.id); accountingToast('Credit note posted successfully'); setPendingAction(null); await load(); }
    catch (error) { accountingToast(errorMessage(error), 'error'); }
    finally { setSaving(false); }
  }
  async function remove(note: CreditNote) {
    setSaving(true); try { await creditNoteApi.remove(note.id); accountingToast('Draft credit note deleted'); setPendingAction(null); await load(); }
    catch (error) { accountingToast(errorMessage(error), 'error'); }
    finally { setSaving(false); }
  }
  ...
  const columns: DashboardTableColumn<CreditNote>[] = [
    ...
    { key: 'status', header: 'Status', align: 'center', cell: (note) => <span className={`${dashboardStatusBadgeClass} ${note.state === 'posted' ? 'bg-emerald-600 text-white' : note.state === 'cancelled' ? 'bg-rose-600 text-white' : 'bg-secondary/100 text-white'}`}>{note.state}</span> },
    { key: 'reference', header: 'Reference', cell: (note) => note.customer_reference || '—' },
    { key: 'actions', header: 'Actions', align: 'right', cell: (note) => <div className="flex justify-end gap-1"><button title="View" onClick={() => edit(note, true)} className={actionBtnView}><Eye className="size-4" /></button>{note.state === 'draft' && <><button title="Edit" onClick={() => edit(note)} className={actionBtnEdit}><SquarePen className="size-4" /></button><button title="Post" onClick={() => setPendingAction({ type: 'post', note })} className={actionBtnView}><Send className="size-4" /></button><button title="Delete" onClick={() => setPendingAction({ type: 'delete', note })} className={actionBtnDelete}><Trash2 className="size-4" /></button></>}{note.state === 'posted' && <button title="Print" onClick={() => print(note)} className={actionBtnView}><Printer className="size-4" /></button>}</div> },
  ];

  return (
    <AccountingPageShell section="Receivables" title="Credit Notes" description="Create customer credits against posted invoices.">
      <DashboardDataTable
        rows={filtered}
        columns={columns}
        loading={loading}
      ...
      <AccountingConfirmDialog open={Boolean(pendingAction)} title={`${pendingAction?.type === 'delete' ? 'Delete' : 'Post'} Credit Note`} description={pendingAction?.type === 'delete' ? 'Confirm removal of this draft credit note.' : 'Confirm this credit note before reducing the original invoice balance.'} confirmLabel={`${pendingAction?.type === 'delete' ? 'Delete' : 'Post'} Credit Note`} destructive={pendingAction?.type === 'delete'} busy={saving} details={pendingAction && <div className="flex justify-between"><span className="text-zinc-500">Credit Note</span><b>{pendingAction.note.invoice_number}</b></div>} onCancel={() => setPendingAction(null)} onConfirm={() => pendingAction && void (pendingAction.type === 'delete' ? remove(pendingAction.note) : post(pendingAction.note))} />
    </AccountingPageShell>
  );
}
