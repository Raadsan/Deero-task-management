...
function departmentName(staff: any) {
  return staff.department?.name ?? staff.departmentName ?? staff.department ?? "N/A";
}
...
export default function StaffTasksSummaryReport() {
  ...
  const rows = useMemo(() => {
    ...
    return users.filter((staff: any) => roleFilter === "all" || String(staff.role).toLowerCase() === roleFilter.toLowerCase()).map((staff: any) => {
      ...
      return { staff, tasks: staffTasks, total: staffTasks.length, pending, completed, overdue };
    }).filter((row: any) => {
      if (row.total === 0) return false;
      return !query || [row.staff.id, row.staff.name, row.staff.role, departmentName(row.staff), portfolioName(row.staff)]
  ...
  const summaryHeaders = ["Staff ID", "Staff Name", "Department", "Role", "Total Tasks", "Processing Tasks", "Completed Tasks", "Overdue Tasks"];
  const summaryExportRows = rows.map((row: any) => [
    valueOrNA(row.staff.id), row.staff.name, departmentName(row.staff), valueOrNA(row.staff.role), row.total, row.pending, row.completed, row.overdue,
  ]);
  ...
  const selected = useMemo(() => {
    ...
    return rows.find((row: any) => String(row.staff.id) === String(selectedStaff.id)) ?? (() => {
      ...
      const pending = staffTasks.filter((task: any) => resolveTaskDisplayStatus(task) === "pending").length;
      const overdue = staffTasks.filter((task: any) => resolveTaskDisplayStatus(task) === "overdue").length;
      return { staff: selectedStaff, tasks: staffTasks, total: staffTasks.length, completed, pending, overdue };
    })();
  ...
  const detailRows = filteredDetailTasks.map(taskDetailRow);
  const paginatedDetailRows = paginatedDetailTasks.map(taskDetailRow);
  function exportSelectedExcel() {
    if (!selected) return;
    exportCsv(`${selected.staff.name}-task-report${dateSuffix}.csv`, detailHeaders, detailRows, [["Period", reportPeriodLabel]]);
  }

  async function exportSelectedPdf() {
    if (!selected) return;
    await exportPdf(`${selected.staff.name}-task-report${dateSuffix}.pdf`, `${selected.staff.name} — Task Report`, detailHeaders, detailRows, {
      subtitle: reportPeriodLabel,
      logoUrl: brandLogo,
      primaryColor: brandColor,
    });
          ...
          <Table>
            <TableHeader className={dashboardTableHeaderClass}>
              <TableRow className={dashboardTableHeadRowClass}>
                {["Staff ID", "Staff Name", "Department", "Role", "Total Tasks", "Processing Tasks", "Completed Tasks", "Overdue Tasks", "Action"].map((header) => (
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={9} className="py-12 text-center text-zinc-500">Loading staff task summary...</TableCell></TableRow>
              ) : rows.length === 0 ? (
                <TableRow><TableCell colSpan={9} className="py-12 text-center text-zinc-500">No staff found</TableCell></TableRow>
                ...
                <TableRow key={row.staff.id} className={dashboardTableBodyRowClass}>
                  ...
                  <TableCell className={dashboardTableCellClass}>{departmentName(row.staff)}</TableCell>
        ...
        <div role="dialog" aria-modal="true" aria-label="Staff task details" className="fixed inset-y-0 right-0 z-50 flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-[2px]" style={{ left: isMobile ? 0 : sidebarOpen ? "16rem" : "5.5rem" }} onMouseDown={() => setSelectedStaff(null)}>
          <div className="max-h-[94vh] w-full max-w-none overflow-y-auto rounded-lg border border-zinc-200 bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between border-b border-zinc-100 px-6 py-5">
              <div><h2 className="text-xl font-bold text-slate-900">Staff Task Details</h2><p className="mt-1 text-sm text-zinc-500">Task summary and breakdown for {selected.staff.name}</p></div>
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-5 border-b border-zinc-100 p-6 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ["ID", `#${selected.staff.id}`],
                ["Name", selected.staff.name],
                ["Department", departmentName(selected.staff)],
                ["Role", selected.staff.role],
                ["Total Tasks", selected.total],
                ["Processing Tasks", selected.pending],
                ["Complete Tasks", selected.completed],
            ...
            <div className="py-6">
              ...
              <div className="w-full overflow-x-auto border-y border-zinc-200">
                <Table className="min-w-[1380px]">
                  ...
                  <TableBody>{paginatedDetailRows.length ? paginatedDetailRows.map((row, index) => <TableRow key={index} className={dashboardTableBodyRowClass}>{row.map((cell, cellIndex) => <TableCell key={cellIndex} className={dashboardTableCellClass}>{cellIndex === row.length - 1 ? <button type="button" aria-label="View task information" onClick={() => setSelectedDetailTask(paginatedDetailTasks[index])} className="inline-flex size-8 items-center justify-center rounded-lg border border-zinc-200 text-primary hover:bg-primary/5"><Eye className="size-4" /></button> : cellIndex === 3 ? <span className={cn("inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold capitalize", getTaskStatusBadgeClass(String(cell)))}>{cell}</span> : cell}</TableCell>)}</TableRow>) : <TableRow><TableCell colSpan={detailHeaders.length} className="py-10 text-center text-zinc-500">No tasks assigned</TableCell></TableRow>}</TableBody>
                </Table>
              </div>
