    ...
    <section className={dashboardCardClass}>
            ...
            <tbody>
              {loading ? Array.from({ length: 5 }, (_, row) => (
                <tr key={row} className={dashboardTableBodyRowClass}>
                  {columns.map((column) => <td key={column.key} className={dashboardTableCellClass}><div className="h-4 animate-pulse rounded bg-zinc-100" /></td>)}
                </tr>
      ...
      <div className={dashboardPaginationClass}>
        ...
        <div className="flex items-center gap-1">
          <button aria-label="Previous page" disabled={currentPage <= 1} onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} className="flex size-7 items-center justify-center rounded-md border border-zinc-200 bg-white text-zinc-500 hover:bg-zinc-50 disabled:opacity-40"><ChevronLeft className="size-3.5" /></button>
          <span className="rounded-md border border-zinc-200 bg-white px-3 py-1.5 text-zinc-500">{currentPage} of {totalPages}</span>
          <button aria-label="Next page" disabled={currentPage >= totalPages} onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} className="flex size-7 items-center justify-center rounded-md border border-zinc-200 bg-white text-zinc-500 hover:bg-zinc-50 disabled:opacity-40"><ChevronRight className="size-3.5" /></button>
        </div>
      </div>
    </section>
  );
